...
class SupabaseManager:
    ...
    def create_sample_emotional_records(self, users):
        ...
        for i in range(0, len(records), batch_size):
            ...
            try:
                result = self.supabase.table('emotional_records').insert(batch).execute()
                if result.data:
    ...
    def create_sample_recommendations(self, users):
        ...
        for paciente in pacientes:
            ...
            selected_templates = random.sample(recommendation_templates, num_recommendations)
            
            for template in selected_templates:
                recommendation = {
                    "user_id": paciente['id'],
                    **template,
                    "completada": random.choice([True, False]) if random.random() < 0.3 else False
                }
                
                if recommendation["completada"]:
                    recommendation["fecha_completada"] = (datetime.now() - timedelta(days=random.randint(1, 7))).isoformat()
    ...
    def get_database_statistics(self):
        ...
        tables = ['users', 'emotional_records', 'assessments', 'recommendations', 'therapy_sessions', 'messages']
    ...
    def export_data_sample(self, filename="supabase_sample_data.json"):
        ...
        try:
            ...
            sample_data['users'] = self.supabase.table('users').select('id,email,nombre,apellido,rol,activo,fecha_registro').limit(10).execute().data
            sample_data['emotional_records'] = self.supabase.table('emotional_records').select('*').limit(50).execute().data
            sample_data['assessments'] = self.supabase.table('assessments').select('*').limit(10).execute().data
            sample_data['recommendations'] = self.supabase.table('recommendations').select('*').limit(20).execute().data
            sample_data['therapy_sessions'] = self.supabase.
